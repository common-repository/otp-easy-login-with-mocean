...
class oelm_Geolocation{
	...
	*/
	...
	public static function get_data( $from_cookie = true ){
		...
		if( $from_cookie && isset( $_COOKIE['oelm_user_ip_data'] ) && !empty( $_COOKIE['oelm_user_ip_data']) ){
			return json_decode( stripslashes( $_COOKIE['oelm_user_ip_data'] ), true );
		}
		...
		if( (!isset( $data['geoplugin_countryCode'] ) || !$data['geoplugin_countryCode'] ) && ( $external_ip_address  = self::get_external_ip_address() ) !== false ){
		...
		@setcookie( 'oelm_user_ip_data', json_encode( $mo_data ) );
