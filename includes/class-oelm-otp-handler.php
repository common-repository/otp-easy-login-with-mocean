...
class oelm_Otp_Handler{
	...
	public function __construct(){
		...
		self::$geoData 		= oelm_Geolocation::get_data();
	...
	*/
	public static function onlySendOTPSMS( $phone_code, $phone_no ){
		...
		if( !$operator ){
			return new Wp_Error( 'no-operator', __( "Operator not found. Please download operator SDK from the plugin settings. Check documentation for how to setup.", 'otp-login-woocommerce' ) );
		}
	...
	*/
	public static function sendOTPSMS( $phone_code, $phone_no ){
		...
		$data = array(
			...
			'expiry' 		=> strtotime( self::$settings['otp-expiry']. ' seconds' ),
			'incorrect' 	=> $incorrect,
	...
	*/
	public static function resendOTPSMS(){
		...
		if( !$phone_otp_data  || !isset( $phone_otp_data[ 'phone_no' ] ) || !$phone_otp_data[ 'phone_no' ] || !isset( $phone_otp_data['phone_code'] ) ){
			return new Wp_Error( 'no-phone', __( "Phone Number not found", 'otp-login-woocommerce' ) );
		}
	...
	public static function generate_otp_digits(){
		$digits = self::$settings['otp-digits'] ? self::$settings['otp-digits'] : 4;
	...
	public static function set_otp_data( $key, $value = '' ){
		...
		update_option( 'oelm_otp_users', $users );
	}


	public static function get_otp_users(){
		return (array) get_option( 'oelm_otp_users' );
	}

	public static function get_otp_data(){
		$users = get_option( 'oelm_otp_users' );
	...
	public static function ok_to_send_otp( $phone_code = '', $phone_no = '' ){
		...
		$resend_limit 		= self::$settings['otp-resend-limit'];
		$incorrect_limit 	= self::$settings['otp-incorrect-limit'];
		$resend_wait_time 	= self::$settings['otp-resend-wait'];
		...
		if( $data['sent_times'] > $resend_limit ){
			...
			if(  $unban_time_left < 0  ){
				...
				return new WP_Error( 'limit-reached', sprintf( __( 'OTP Limit reached. Please try again in %s.', 'otp-login-woocommerce' ), self::getTimeDuration( $unban_time_left) ) );
			}
		}
		...
		if( $data['phone_no'] === $phone_no && $data['phone_code'] === $phone_code && $data['sent_times'] >= 1 &&  $resend_wait_time > $time_passed ){
			...
			return new WP_Error( 'resend-wait', sprintf( __( 'Please wait %s for a new OTP.', 'otp-login-woocommerce' ), self::getTimeDuration( $unban_time_left) ) );
		}
		...
		if( $data['incorrect'] >= $incorrect_limit ){
			...
			if( $unban_time_left < 0 ){
				...
				return new WP_Error( 'tries-exceeded', sprintf( __( 'Maximum number of tries exceeded. Please try again in %s.', 'otp-login-woocommerce' ), self::getTimeDuration( $unban_time_left) ) );
			}
		}
	...
	public static function getOTPSMSText( $otp ){
		...
		$sms_text = self::$settings['r-sms-txt'];
		...
		$sms_text = apply_filters( 'oelm_phone_sms_text',$sms_text );
	...
	*/
	public static function get_operator(){
		$service = self::$settings['m-operator'];

		switch ( $service ) {
			case 'mocean':
				$operatorFunc = 'oelm_mocean';
				break;
	...
	*/
	public static function cleanup(){
		...
		$last_cleanup = (int) get_option( 'oelm_last_cleanup', true );

		$now = time();

		//Check if cleanup was done 24 hours ago
		if( ( $now - $last_cleanup ) < ( apply_filters( 'oelm_cleanup_last_done', 24 ) * 1 ) ){
			return;
		}
		...
		update_option( 'oelm_otp_users', $users );
		update_option( 'oelm_last_cleanup', $now );

	}
}

function oelm_otp_handler(){
	return oelm_Otp_Handler::get_instance();
}
oelm_otp_handler();